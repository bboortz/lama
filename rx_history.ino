...
void addToRxHistory(String user, int seq, int msecs, float snr, float rssi, float freqErr, float rsnr) {
...
void updateUserSignalStats(UserStats* stats, float rssi, float snr, float freqErr) {
  ...
  // Min/Max
  if (rssi < stats->minRssi || stats->received == 1) stats->minRssi = rssi;
...
void printUserStats() {
  ...
  for (int i = 0; i < userCount; i++) {
    ...
    Serial.printf("  RSSI:     %.0f dBm (avg: %.0f, min: %.0f, max: %.0f)\n", 
                  s->lastRssi, s->avgRssi, s->minRssi, s->maxRssi);
    Serial.printf("  SNR:      %.1f dB (avg: %.1f)\n", s->lastSnr, s->avgSnr);
    Serial.printf("  FreqErr:  %.0f Hz (avg: %.0f)\n", s->lastFreqErr, s->avgFreqErr);
    
    // Link quality assessment
    String quality;
    if (s->avgRssi > -85 && lossPct < 5) quality = "Excellent";
    else if (s->avgRssi > -100 && lossPct < 10) quality = "Good";
    else if (s->avgRssi > -110 && lossPct < 20) quality = "Fair";
    else quality = "Poor";
